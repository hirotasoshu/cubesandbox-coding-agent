...
Only `network_policy.py` uses the native `cubesandbox` package because typed
CubeEgress rules and credential injection are Cube-specific.
...
The flag is available on every E2B-based example in the tables below. It is
not available on `network_policy.py`, which uses the native CubeSandbox SDK and
its own `CUBE_PROXY_NODE_IP` transport. The sidecar is development-only and
...
Append `--dev-sidecar` to any command above except `network_policy` when using
the optional development proxy.
...
- The two native network-policy examples put only a placeholder token in the
